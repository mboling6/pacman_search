...
class CornersProblem(search.SearchProblem):
    ...
    def getSuccessors(self, state):
        ...
        for action in [Directions.NORTH, Directions.SOUTH, Directions.EAST, Directions.WEST]:
            ...
            if not hitsWall: 
                next = (nextx, nexty)
                for counter, corner in enumerate(self.corners):
                    if next == corner:
                        cornersVisited = list(cornersVisited[:])
                        cornersVisited[counter] = True
                        cornersVisited = tuple(cornersVisited)
                successorState = tuple((next, cornersVisited))
...
def cornersHeuristic(state, problem):
    ...
    "*** YOUR CODE HERE ***"
    cornersUnvisited = []
    for counter, hit in enumerate(state[1]):
        if hit == False:
            cornersUnvisited.append(corners[counter])
    heuristic = 0
    currentPosition = state[0]

    while cornersUnvisited:
        cornersChecked = []
        for counter, corner in enumerate(cornersUnvisited):
            xy1 = currentPosition
            xy2 = cornersUnvisited[counter]
            cornersChecked.append( (abs(xy1[0] - xy2[0]) + abs(xy1[1] - xy2[1]), counter) )

        mins = min(cornersChecked, key = lambda t: t[0])

        heuristic += mins[0]
        currentPosition = cornersUnvisited[mins[1]]
        del cornersUnvisited[mins[1]]
    ...
    # distances = [ ]
    # for corner2 in cornersUnvisited:
    #     distances.append(util.manhattanDistance(currentPosition, corner2))
    
    # maxDistance = 0
    # for corner1 in cornersVisited: 
    #     for corner2 in cornersVisited: 
    #         distance = util.manhattanDistance(corner1, corner2)
    #         maxDistance = max(maxDistance, distance)
    
    # heuristic = min(distances) + maxDistance
...
def foodHeuristic(state, problem):
    ...
    minDistance = util.manhattanDistance(position, foodGridList[0])
